...
export default function AdminDashboard() {
  ...
  const mainStats = [
    ...
    { title: "Toplam Öğrenci", value: stats?.totalStudents || 0, trend: "+8.2%", icon: BookOpen, color: "text-green-600", bg: "bg-green-50" },
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-[var(--text-primary)] tracking-tight">Yönetim Paneli</h1>
          <p className="text-slate-500 dark:text-[var(--text-muted)] font-medium">Hoş geldiniz, platformun genel performansını buradan takip edebilirsiniz.</p>
        </div>
        <div className="flex gap-3">
          <Button 
            variant="outline" 
            className="rounded-xl border-slate-200 dark:border-[var(--card-border)] dark:text-[var(--text-primary)] font-bold flex items-center gap-2 hover:bg-slate-50 dark:hover:bg-slate-800"
            onClick={handleExport}
          >
            <Download size={18} /> Rapor İndir
          </Button>
          <Button className="rounded-xl bg-slate-900 dark:bg-green-600 hover:bg-black dark:hover:bg-green-700 text-white font-bold flex items-center gap-2 px-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {mainStats.map((stat, i) => (
          <Card key={i} className="border-none shadow-sm bg-white dark:bg-[var(--card-bg)] rounded-3xl overflow-hidden hover:shadow-xl dark:hover:shadow-none transition-all border border-slate-100/50 dark:border-[var(--card-border)]/50">
            <CardContent className="p-7">
              ...
              <div className="text-3xl font-black text-slate-900 dark:text-[var(--text-primary)] mb-1">{stat.value}</div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        {/* Activity Chart */}
        <Card className="lg:col-span-2 border-none shadow-sm bg-white dark:bg-[var(--card-bg)] rounded-[2.5rem] overflow-hidden transition-colors">
          <CardHeader className="p-8 border-b border-slate-50 dark:border-[var(--card-border)] flex flex-row items-center justify-between">
            <CardTitle className="text-xl font-black text-slate-900 dark:text-[var(--text-primary)]">Platform Trafiği</CardTitle>
            <div className="flex gap-2">
              {['Haftalık', 'Aylık'].map(t => (
                <button key={t} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-colors ${t === 'Aylık' ? 'bg-slate-900 dark:bg-green-600 text-white' : 'bg-slate-100 dark:bg-[var(--card-bg)] text-slate-500 dark:text-[var(--text-muted)] hover:bg-slate-200 dark:hover:bg-slate-700'}`}>{t}</button>
              ))}
            </div>
          </CardHeader>
                  ...
                  return (
                    <div key={i} className="flex-1 flex flex-col items-center gap-2 group">
                      <div className="relative w-full">
                        <div 
                          className={`w-full rounded-t-lg transition-all duration-500 group-hover:bg-green-600 ${i % 2 === 0 ? 'bg-slate-100 dark:bg-[var(--card-bg)]' : 'bg-green-500'}`} 
                          style={{ height: `${Math.max(height, 5)}%` }}
                        ></div>
                        <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-900 dark:bg-green-600 text-white text-[10px] py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-20">
                          {val} Kayıt
                        </div>
                      </div>
           ...
           <CardContent className="p-8 space-y-8">
              <div className="space-y-4">
                 ...
                 <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                    <div 
                      className="h-full bg-green-500 rounded-full shadow-[0_0_10px_rgba(59,130,246,0.5)] transition-all duration-1000"
